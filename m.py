...
bot = telebot.TeleBot('7380335556:AAGzulMWKTqXy1vFPaDugYxrKDoB9SqbmUY')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 2:
                response = "𝙔𝙊𝙐 𝘼𝙍𝙀 𝙊𝙉 𝘾𝙊𝙊𝙇𝘿𝙊𝙒𝙉 𝙋𝙇𝙀𝘼𝙎𝙀 𝙒𝘼𝙄𝙏 300 𝙎𝙀𝘾𝙊𝙉𝘿𝙎 ❌."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 501:
                response = "𝙀𝙍𝙍𝙊𝙍 : 𝙏𝙄𝙈𝙀 𝙄𝙉𝙏𝙀𝙍𝙑𝙀𝙇 𝙈𝙐𝙎𝙏 𝘽𝙀 𝙇𝙀𝙎𝙎 𝙏𝙃𝙀𝙉 80."
            else:
